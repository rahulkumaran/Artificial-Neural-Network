...
def sin_problem():
    ...
    hidden_layer = ann.tanhx(np.dot(parameters["W1"],test_ip.T) + parameters["b1"])
    output_layer = ann.tanhx(np.dot(parameters["W2"],hidden_layer) + parameters["b2"])

    all_out = np.append(all_out, output_layer)
    all_in = np.append(all_in, test_ip)

    plt.plot(test["x"],test["sin_x"],'r')
...
class ANN(object):
    ...
    def cost_function(self, predictions, outputs):
        print("found cost")
        self.train_cost = (1/700)*(outputs-predictions)**2/2
        return (1/700)*(outputs-predictions)**2/2

    def cost_validation(self, predictions, outputs):
        print("found cost")
        self.val_cost = (1/700)*((outputs-predictions)**2/2)
        return (1/700)*((outputs-predictions)**2/2)

    def backprop(self, actual_outputs, outputs, activation_cache):
        L = len(self.parameters)//2
        self.grads["dZ" + str(L)] = outputs - actual_outputs
        self.grads["dW" + str(L)] = (1/700)*np.dot(self.grads["dZ" + str(L)], activation_cache[L-1].T)
        self.grads["db" + str(L)] = (1/700)*np.sum(self.grads["dZ" + str(L)], axis = 1, keepdims = True)
        ...
        for layer in range(L-1, 0, -1):
            ...
            self.grads["dW" + str(layer)] = (1/700)*np.dot(self.grads["dZ" + str(layer)], activation_cache[layer - 1].T)
            self.grads["db" + str(layer)] = (1/700)*np.sum(self.grads["dZ" + str(layer)], axis = 1, keepdims=True)
    ...
    def update_parameters(self, learning_rate):
        ...
        for i in range(0, len(self.parameters)//2):
            ...
            self.parameters["W" + str(i+1)] = self.parameters["W" + str(i+1)] - dW
...
if(__name__ == "__main__"):
    ...
    parameters = train_model(train_x, train_y, [4,20,1])

    hidden_layer = tanhx(np.dot(parameters["W1"],input_layer.T) + parameters["b1"])
    output_layer = tanhx(np.dot(parameters["W2"],hidden_layer) + parameters["b2"])
